...
// Печатает в текущей позиции содержимое GDTR
// Печатать нужно следующее: 
// - сначала 16-ричном виде все содержимое (на отдельной строке)
// - потом отдельной строкой в двоичном виде все содержимое (с разбивкой по байтам)
// - разбивка по полям (описание поля и его значение) 
void print_gdtr() {
    struct gdtr gdtr;
    __asm__ volatile("sgdt %0" : "=m"(gdtr));
    char s[20];
    itohexs(gdtr.limit, 8, s);
    puts(s);
    putc(' ');
    itohexs(gdtr.base, 8, s);
    puts(s);
    putc('\n');
    for (int i = 15; i >= 0; i--) {
        putc(((gdtr.limit >> i) & 1) + '0');
        if (i % 4 == 0)
            putc(' ');
    }
    for (int i = 31; i >= 0; i--) {
        putc(((gdtr.base >> i) & 1) + '0');
        if (i % 4 == 0)
            putc(' ');
    }
    putc('\n');
...
// Печатает в текущей позиции содержимое участка GDT
// start - номер начальной строки
// n - количество строк
// Если заказаны номера строк, которых реально в таблице нет
// (согласно содержимому GDTR), напечатаем разумное сообщение про это
// Для каждой существующей заказанной записи печатать нужно следующее: 
// - сначала 16-ричном виде все содержимое (на отдельной строке)
// - потом отдельной строкой в двоичном виде все содержимое (с разбивкой по байтам)
// - разбивка по полям (описание поля и его значение)
void print_gdt(unsigned int start, unsigned int n) {
    struct gdtr gdtr;
    __asm__ volatile("sgdt %0" : "=m"(gdtr));  // Загружаем GDTR

    if ((start + n) * 8 > gdtr.limit + 1) {
        puts("error: there are no such lines in gdt\n");
        return;
    }

    for (unsigned int i = start; i < start + n; i++) {
        struct gdt* entry = (struct gdt*)(gdtr.base + i * 8);

        char buffer[20];
        itohexs(entry->limit_low, 4, buffer);
        puts(buffer);
        putc(' ');

        itohexs(entry->base_low, 4, buffer);
        puts(buffer);
        putc(' ');

        itohexs(entry->base_middle, 2, buffer);
        puts(buffer);
        putc(' ');

        itohexs(entry->access, 2, buffer);
        puts(buffer);
        putc(' ');
        
        itohexs(entry->flag, 2, buffer);
        puts(buffer);
        putc(' ');
        
        itohexs(entry->base_high, 2, buffer);
        puts(buffer);
        putc('\n');

        for (int j = 15; j >= 0; j--) {
            putc(((entry->limit_low >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        for (int j = 15; j >= 0; j--) {
            putc(((entry->base_low >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        for (int j = 7; j >= 0; j--) {
            putc(((entry->base_middle >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        for (int j = 7; j >= 0; j--) {
            putc(((entry->access >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        for (int j = 7; j >= 0; j--) {
            putc(((entry->flag >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        for (int j = 7; j >= 0; j--) {
            putc(((entry->base_high >> j) & 1) + '0');
            if (j % 4 == 0)
                putc(' ');
        }
        putc('\n');
        ...
        puts("limit low: ");
        itos(entry->limit_low, dec_buffer);
        puts(dec_buffer);
        putc('\n');
        
        puts("base low: ");
        itos(entry->base_low, dec_buffer);
        puts(dec_buffer);
        putc('\n');
        
        puts("base middle: ");
        itos(entry->base_middle, dec_buffer);
        puts(dec_buffer);
        putc('\n');
        
        puts("access: ");
        itos(entry->access, dec_buffer);
        puts(dec_buffer);
        putc('\n');
        
        puts("flag: ");
        itos(entry->flag, dec_buffer);
        puts(dec_buffer);
        putc('\n');

        puts("base high: ");
        itos(entry->base_high, dec_buffer);
        puts(dec_buffer);
        putc('\n');
    }
}
